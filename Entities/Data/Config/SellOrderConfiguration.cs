...
public class SellOrderConfiguration : IEntityTypeConfiguration<SellOrder>
{
    public void Configure(EntityTypeBuilder<SellOrder> builder)
    {
        ...
        builder.Property(so => so.StockSymbol).HasMaxLength(20).IsRequired();

        builder.Property(so => so.StockName).HasMaxLength(100).IsRequired();

        builder.Property(so => so.DateAndTimeOfOrder).HasColumnType("datetime2").IsRequired();

        builder.Property(so => so.Quantity).IsRequired();

        builder.Property(so => so.Price).HasPrecision(15, 2).IsRequired();
